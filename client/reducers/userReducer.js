...
const userReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case types.COMPLETE_ORDER:
      console.log('COMPLETE ORDER REDUCER CALLED', action.payload);

      let orderNum = action.payload.orderId * 1;
      let ordersCopy = state.orders.slice();

      ordersCopy = state.orders.filter(order => {
        console.log('orderNum ', orderNum)
        console.log('order ', order['fk_orders'])
        return orderNum !== order['fk_orders']
      })
      ...
      for (let i = 0; i < ordersCopy.length; i += 1) {
        ...
        if (orderNum !== order['fk_orders']) {
          console.log('pushingggg ', order)
          result.push(order);
        }
      }
      console.log('changing state to ', result)
      return {
        ...state,
        orders: result,
      };
